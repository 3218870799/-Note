...
` HystrixCommandProperties.java ` 中都在；

使用注解

```java
@HystrixCommand(commandProperties = {
    @HystrixProperty(name="circutBreaker.enabled",value="true")
},fallbackMethod = "fallback")
```
...
方法级别，方法名参数类型，个数都要和被降级方法保持一致；这样调用fallback方法会单独启用一个线程池；

也可以放在类级别，将注解标在类型

使用Feign时，一个Feign客户端一个fallback，这样可以使fallback方法和调用远程方法公用一个线程池，减少消耗；
...
触发熔断与自动恢复：
 	...
 	3,在我们的时间窗口期期间,尝试让一些请求通过(半开状态),如果请求还是失败,证明断路器还是开启状态,服务没有恢复，如果请求成功了,证明服务已经恢复,断路器状态变为close关闭状态
...
```xml
<dependency>
    <groupId>org.springframenwork.cloud</groupId>
    <artifactId>spring-cloud-starter-zipkin</artifactId>
</dependency>
```
