...
这个就是 java8 中的 Integer 类中的一个内部缓存类（其他版本的jdk实现有可能不一样但是效果都是一样的），这个类的作用就是将 -128~127 之间的整型做了一个缓存，（享元模式）
...
根据元素的 hashCode值来决定元素的存储位置， 但它同时使用双向链表维护元素的次序，这使得看起来是以 插入 顺序保存,不允许重复
...
TreeSet会调用集合元素的compareTo(Objecto)方法来比较元素之间的大小关系，然后将集合元素按升序排列，这种方式是自然排序。底层使用红黑树结构存储数据。
...
TreeMap是用键来进行升序顺序来排序的。底层也是使用红黑树结构存储数据，通过Comparable 或 Comparator来排序。（实现和TreeSet基本一致）。
...
继承Thread类实现线程，扩展性不强，因为Java类只能继承一个
...
t.start()才会启动一个线程，通过调用Thread类的start()方法来启动一个线程，这时此线程处于就绪（可运行）状态，并没有运行，一旦得到cpu时间片，就开始执行run()方法，这里方法 run()称为线程体，它包含了要执行的这个线程的内容，Run方法运行结束，此线程随即终止。
...
（2）并发不高、任务执行时间长的业务要区分开看：分为CPU密集型和IO密集型。性质不同的任务可用使用不同规模的线程池分开处理：  
...
ReentantLock相比于Synchronized可以更方便的获取锁，可以操作读写锁，可以唤醒指定线程等等，
...
2、如果使用的是有界队列比如 ArrayBlockingQueue，任务首先会被添加到ArrayBlockingQueue 中，ArrayBlockingQueue 满了，会根据maximumPoolSize 的值增加线程数量，如果增加了线程数量还是处理不过来，ArrayBlockingQueue 继续满，那么则会使用拒绝策略，RejectedExecutionHandler 处理满了的任务，默认是 AbortPolicy
...
流 程： 
- DatagramSocket与DatagramPacket 2. 
- 建立发送端，接收端 3.
-  建立数据包 4. 
- 调用Socket的发送、接收方法 5. 
- 关闭Socket
...
提供了最高级网络应用。URL 的网络资源的位置来同一表示 Internet 上各种网络资源。通过URL对象可以创建当前应用程序和 URL 表示的网络资源之 间的连接，这样当前程序就可以读取网络资源数据，或者把自己的数据传送到网络上去。
...
枚举类对象的属性不允许被改动，应该使用private final修饰，且应在构造器中为其赋值。私有化构造器，保证不能在类外部创建对象
...
1.使用类型通配符：？ 比如：List ，Map List是List、List等各种泛型List的父类。 
