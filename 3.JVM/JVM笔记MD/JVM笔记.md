...
（4） 虚引用（PhantomReference） 必须配合引用队列使用，主要配合 ByteBuffer 使用，被引用对象回收时，会将虚引用入队， 由 Reference Handler，虚引用根本get不到。回收时会去队列里检查是否还有引用。

线程调用虚引用相关方法释放直接内存。

主要用来管理直接内存，即堆外内存。NIO中零拷贝，会将资源拷贝到堆外内存buffer，JVM要想使用buffer，要么拷贝到虚拟机堆内，要么使用虚引用进行直接操作。
