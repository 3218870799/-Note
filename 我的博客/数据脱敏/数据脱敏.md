...
2：有部分导入的功能，导出的数据也可能需要模糊掉；新增时不能进行加密；
...
2：自定义一个注解，将需要进行模糊类型统一封装成一个实体，让需要脱敏的返回类型继承该实体，这样可以避免每一个实体中都去添加注解，然后进行AOP编程，将数据进行模糊处理；

切面的处理几乎每个都要搞，将其需要进行处理的权限字段放入到Redis中，修改权限控制时删除并更新Redis；



![image-20211012152951449](media/image-20211012152951449.png)
...
https://www.cnblogs.com/sun-flower1314/p/15117630.html
