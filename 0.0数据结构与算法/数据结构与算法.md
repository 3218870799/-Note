...
> 判断单链表为空的条件是 head-\>next==NULL，而判断循环单链表为空的条件是 head-\>next==head。访问第一个结点即 rear-\>next-\>next。
...
AVL树：左右子树都是AVL树，且左右子树高度差不超过 1；

平衡因子BF = 左子树深度-右子树深度 =(-1,0,1)
...
问题1：从6*9的方格，从左上走到右下，每次只能往下往右走，请问有多少种走法？
...
![image-20210420135544883](media/image-20210420135544883.png)
...
异或：x \^ y：相同为 0，不同为 1；**N与0异或结果为N；N与N异或结果为0；**异或复合交换结合律，先后顺序并不影响；
...
网页黑名单；垃圾邮件过滤；允许一部分的错误；对空间有一定的要求；

失误率取决于hash函数；

bitArray确定：由样本数量N和失误率p来决定；单个样本的大小不会影响布隆过滤器的大小；

m = - （N* ln P）/(ln2)^2；

哈希函数的个数 = ln2 * (m/n)

![image-20210711145258764](media/image-20210711145258764.png)

![image-20210711145338895](media/image-20210711145338895.png)
